...
class CatalogTests(TestCase):
    ...
    def test_build_is_reproducible_with_fixed_time(self):
        expected_count = len(list((ROOT / "catalog" / "approved").glob("*.json")))
        expected_skill_count = sum(1 for path in (ROOT / "plugins").rglob("SKILL.md"))
        ...
        self.assertEqual(sum(1 for entry in payload["entries"] for file in entry["integrity"]["files"] if file["path"].endswith("SKILL.md")), expected_skill_count)
    ...
    def test_build_writes_one_click_import_descriptor(self):
        ...
        self.assertEqual(descriptor["marketplace"]["plugin_count"], len(list((ROOT / "catalog" / "approved").glob("*.json"))))
    ...
    def test_build_writes_federated_source_descriptor(self):
        ...
        research = {item["id"]: item for item in source["featured_plugins"]}
        self.assertTrue({"scispace", "consensus", "sider-scholar", "scite", "elicit"}.issubset(research))
        self.assertTrue(all(item["summary_zh"] and item["search_terms_zh"] for item in research.values()))
    ...
    def test_homepage_explains_two_plugin_sources(self):
        ...
        self.assertIn("renderOfficialFeatured", script)
        self.assertIn("论文检索、文献综述与引文核验", html)

    def test_chinese_academic_research_plugin_is_packaged(self):
        entry = json.loads((ROOT / "catalog" / "approved" / "academic-research-toolkit.json").read_text(encoding="utf-8"))
        manifest = json.loads((ROOT / "plugins" / "academic-research-toolkit" / ".codex-plugin" / "plugin.json").read_text(encoding="utf-8"))
        candidate = json.loads((ROOT / "catalog" / "candidates" / "academic-research-toolkit-9b1b256e9905.json").read_text(encoding="utf-8"))
        self.assertEqual(entry["name"], "中文学术研究工具箱")
        self.assertEqual(entry["category"], "教育科研")
        self.assertEqual(entry["license"]["spdx"], "MIT")
        self.assertEqual(candidate["scan"], {"file_count": 124, "findings": [], "total_bytes": 431946, "verdict": "review-required"})
        self.assertEqual(sum(item["path"].endswith("SKILL.md") for item in candidate["files"]), 10)
        self.assertEqual(manifest["interface"]["displayName"], "中文学术研究工具箱")
        self.assertTrue(all(any("\u4e00" <= char <= "\u9fff" for char in prompt) for prompt in manifest["interface"]["defaultPrompt"]))
        readme_zh = (ROOT / "plugins" / "academic-research-toolkit" / "README.zh-CN.md").read_text(encoding="utf-8")
        self.assertIn("直接这样问", readme_zh)
        self.assertIn("上游内容保持原样", readme_zh)
