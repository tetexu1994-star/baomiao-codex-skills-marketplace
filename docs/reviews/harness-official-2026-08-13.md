...
- 交付：其中 52 个目录封装为独立 Codex 插件，进入市场清单；每个包保留仓库级 Apache-2.0 许可证，并与固定提交的候选摘要逐文件核对。
...
本轮审核了 55 个目录，将其中 52 个封装为插件发布。它们来自同一个具名厂商官方仓库、许可证范围一致、目录结构一致且无可执行文件。没有把“官方”当作低风险依据；所有会改变 Harness 远端状态的 Skill 均按增强权限发布。
